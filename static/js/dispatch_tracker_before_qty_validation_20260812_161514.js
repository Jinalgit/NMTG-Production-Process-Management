    ...
    custEl.querySelectorAll('.dt-so').forEach(soEl => {
      ...
      if (term) {
        const hay = [
          ...
          soEl.dataset.so,
          ...Array.from(soEl.querySelectorAll('.dt-wo')).map(w => w.dataset.wo),
          ...Array.from(soEl.querySelectorAll('.dt-jc')).map(j => j.dataset.jc),
        ].join(' ').toLowerCase();
        if (!hay.includes(term)) soVisible = false;
        else {
