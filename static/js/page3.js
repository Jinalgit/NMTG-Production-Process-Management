...
async function fetchJobCard() {
  ...
  try {
    ...
    if (IS_OPERATOR_READ_ONLY) {
       ...
       * for immediately incoming CNC/VMC JCs.
       *
       * Active Jobs itself remains current-WIP only.
       */
      const queueRes = await fetch(
        "/api/operator/job_cards?include_incoming=1"
      );
      ...
      if (!queueData.success) {
        showToast(
          queueData.error
          || "Unable to check operator queue.",
          "error"
        );
        return;
      }
      ...
      /*
       * /api/quality_check/fetch already resolves
       * the actual JMS JC number.
       *
       * Therefore a search such as:
       *
       *     1234
       *
       * can continue using:
      ...
      const actualJc = String(
        data?.job_card?.job_card_no
        || jcNo
      ).trim();


      const queueCards =
        Array.isArray(queueData.job_cards)
          ? queueData.job_cards
          : [];
      ...
      /*
       * CASE 1:
       * JC is already at this operator's
       * current CNC/VMC process.
       *
       * Keep existing completion/OEE modal.
       */
      const currentCards =
        queueCards.filter(
          function(card) {
            return card?.is_incoming !== true;
          }
        );
      ...
      if (currentIdx >= 0) {
        ...
        operatorCards = currentCards;
        operatorPage = 0;

        openOperatorCompleteModal(
          currentIdx
        );
      ...
      /*
       * CASE 2:
       * JC is still at the immediately
       * previous process, and its NEXT
       * process belongs to this operator.
       *
       * Open proper Receive Material modal.
       */
      const incomingCard =
        queueCards.find(
          function(card) {
      ...
      if (incomingCard) {
        ...
        openOperatorReceiveMaterialModalV3(
          incomingCard
        );
      ...
      /*
       * Not current and not immediately incoming.
       */
      showToast(
        "This Job Card is not at your current process "
        + "or immediately incoming to your assigned process.",
        "error"
      );
...
/* OPERATOR_SEARCH_RECEIPT_MODAL_V3_START */

let operatorReceiveMaterialCardV3 = null;


function operatorIncomingRequiredQtyV3(card) {

  const jcQty = Math.max(
    0,
    operatorNumber(
      card?.job_card_qty,
      0
...
function operatorCloseReceiveMaterialModalV3() {

  operatorReceiveMaterialCardV3 = null;

  const overlay = document.getElementById(
    "operator-receive-material-overlay-v3"
  );
...
function openOperatorReceiveMaterialModalV3(card) {

  operatorCloseReceiveMaterialModalV3();

  operatorReceiveMaterialCardV3 = card;


  const requiredQty =
    operatorIncomingRequiredQtyV3(
      card
    );


  const previousProcess = String(
    card?.previous_process_to_complete
    || card?.current_process
    || card?.wip_status
    || ""
  ).trim();
  ...
  overlay.innerHTML = `
    <div
      role="dialog"
      aria-modal="true"
      aria-labelledby="operator-receive-title-v3"
      style="
        width:min(560px,100%);
        max-height:92vh;
        overflow:auto;
    ...
    >
      ...
      ">
        ...
        <div style="flex:1;min-width:0;">
          ...
          <div style="
            ...
            font-size:12px;
          ">
            Confirm material received from the
            immediately previous process
          </div>
        ...
        <button
          type="button"
          onclick="operatorCloseReceiveMaterialModalV3()"
          style="
            width:36px;
            height:36px;
            border:1px solid #e2e8f0;
      ...
      <div style="padding:22px;">
        ...
        ">
          ...
          <strong>${esc(previousProcess)}</strong>
          will be completed and this Job Card
          will move to
          <strong>${esc(incomingProcess)}</strong>.
      ...
      ">
        ...
        <button
          type="button"
          id="operator-receive-submit-v3"
          onclick="operatorSubmitReceiveMaterialV3()"
          ${requiredQty <= 0 ? "disabled" : ""}
          style="
            padding:10px 20px;
            border:0;
            border-radius:10px;
...
async function operatorSubmitReceiveMaterialV3() {

  const card =
    operatorReceiveMaterialCardV3;


  if (!card) {
    showToast(
      "Incoming Job Card information is missing.",
      "error"
    );
    return;
  }
  ...
  if (requiredQty <= 0) {
    showToast(
      "JC quantity is zero. Supervisor review is required.",
      "error"
    );
    return;
  }
  ...
  if (receivedQty !== requiredQty) {
    showToast(
      `Received Qty (${receivedQty}) must match `
      + `full JC Qty (${requiredQty}).`,
      "error"
    );
  ...
  if (!incomingProcess) {
    showToast(
      "Incoming process information is missing.",
      "error"
    );
    return;
  }
  ...
  try {
    ...
    showToast(
      ...
      + `Current WIP: ${result.new_stage || incomingProcess}`,
      "success"
    );


    /*
     * Refresh Active Jobs only.
     * Incoming list no longer exists.
     */
    if (
      typeof operatorRefreshWorkspaceV2
      === "function"
    ) {
      await operatorRefreshWorkspaceV2(
        false
      );
...
/* OPERATOR_SEARCH_RECEIPT_MODAL_V3_END */

/* OEE_LIVE_CALC_PREVIEW_V1_START */


/*
 * IMPORTANT:
 *
 * This is DISPLAY calculation only.
 *
 * Final saved OEE is still calculated by the
 * Python backend using:
 *
 *   routes/oee_calculations.py
 *   routes/oee_persistence.py
 *
 * Therefore this preview never becomes the
 * database authority.
 */


function operatorOeeNumberV1(id) {
...
function operatorOeeTimeMinutesV1(value) {
...
function operatorOeePlannedMinutesV1() {

  const startElement =
    document.getElementById(
      "operator-oee-start-time"
    );

  const endElement =
    document.getElementById(
      "operator-oee-end-time"
    );


  const start =
    operatorOeeTimeMinutesV1(
      startElement?.value
    );
...
function operatorOeeMachineProfileV1() {

  const select =
    document.getElementById(
      "operator-oee-machine"
    );
  ...
  if (machineNo === "CNC 32") {
    return {
      ...
      machineNo: machineNo,
      error:
        "CNC 32 Excel OEE formula profile is still under review."
    };
  }


  const p06Machines = new Set([
    "CNC 15",
    "CNC 17",
    "VMC 01",
    "VMC 02",
  ...
  return {
    ...
    machineNo: machineNo,
    error:
      `No confirmed Excel OEE formula profile for ${machineNo}.`
  };
}


function operatorOeeLossMapV1() {
...
function operatorOeeFormatNumberV1(
  value,
  digits = 2
) {
...
function operatorOeeFormatPercentV1(
  value
) {
  ...
  /*
   * Do NOT cap at 100%.
   * Excel detailed block has no such cap.
   */
  return (
    value * 100
  ).toFixed(2) + "%";
...
function operatorCalculateOeePreviewV1() {

  const profile =
    operatorOeeMachineProfileV1();
  ...
  if (!profile) {
    return {
      ready: false,
      message:
        "Select Machine to calculate OEE."
    };
  }
  ...
  if (
    ...
    return {
      ready: false,
      message:
        "Cycle / Load seconds must be between 0 and 59."
    };
  }
  ...
  const losses =
    operatorOeeLossMapV1();


  const arLoss =
    profile.arCodes.reduce(
      (sum, code) =>
        sum + (losses[code] || 0),
      0
    );
  ...
  /*
   * Match current backend exactly:
   *
   * Target =
   * (Planned - machine-profile deduction)
   * / Ideal Cycle
   */
  const targetQty =
    idealCycle !== 0
...
function operatorRenderOeePreviewV1() {

  const host =
    document.getElementById(
      "operator-oee-live-preview-v1"
    );
  ...
  if (!calc.ready) {
    ...
    host.innerHTML = `
      <div style="
        ...
        text-align:center;
      ">
        ${esc(calc.message || "Enter OEE details.")}
      </div>
    `;
  ...
  host.innerHTML = `
    ...
    ">
      ...
      ">
        ...
        ${operatorOeeMetricHtmlV1(
          "Planned Min",
          operatorOeeFormatNumberV1(
            calc.planned
          )
        )}
      ...
      <div style="
        ...
        line-height:1.4;
      ">
        Preview only. Final OEE is recalculated
        and saved by the Python backend.
      </div>
...
function operatorOeeMetricHtmlV1(
  label,
  value,
  highlight = false
) {
...
function operatorInitOeeLivePreviewV1() {

  const section =
    document.getElementById(
      "operator-oee-section"
    );
  ...
  if (
    ...
    const recalc =
      function(event) {
        ...
        const isRelevant =
          id === "operator-oee-machine"
          ||
          id === "operator-oee-start-time"
          ||
          id === "operator-oee-end-time"
          ||
          id === "operator-oee-cycle-min"
          ||
          id === "operator-oee-cycle-sec"
          ||
          id === "operator-oee-load-min"
          ||
          id === "operator-oee-load-sec"
          ||
          id === "operator-ok-qty"
          ||
          id === "operator-notok-qty"
    ...
    /*
     * Qty fields sit outside OEE section.
     */
    [
      "operator-ok-qty",
      "operator-notok-qty",
  ...
  /*
   * Loss master / machine list loads asynchronously.
   * Re-render after master data has had time to populate.
   */
  window.setTimeout(
    operatorRenderOeePreviewV1,
    300
...
/*
 * Enhance existing completion modal without
 * replacing its current workflow.
 */
const operatorOpenCompleteModalBaseOeeCalcV1 =
  openOperatorCompleteModal;


openOperatorCompleteModal =
  function openOperatorCompleteModalWithOeeCalcV1(
    globalIdx
  ) {

    operatorOpenCompleteModalBaseOeeCalcV1(
      globalIdx
    );
    ...
    window.setTimeout(
      function() {
        ...
        if (
          card
          &&
          operatorOeeIsApplicable(
            card
          )
        ) {
          operatorInitOeeLivePreviewV1();
        }
...
/* OEE_LIVE_CALC_PREVIEW_V1_END */

/* CNC_VMC_MACHINE_SELECTION_VIEW_V1_START */
 ...
 * PAGE 3 - THIRD VIEW
 *
 * 1. Supervisor              -> existing view
 * 2. Normal Operator         -> existing view
 * 3. CNC/VMC Operator        -> this machine selection view
 *
 * There is NO operator-zone assignment.
 *
 * Zone comes only from oee_machines.zone.
 * ==========================================================
 */


const CNC_VMC_SELECTED_MACHINE_KEY_V1 =
  "jms_cnc_vmc_selected_machine_v1";


const CNC_VMC_SELECTED_ZONE_KEY_V1 =
  "jms_cnc_vmc_selected_zone_v1";


let cncVmcMachineSelectionDataV1 =
  null;


let cncVmcActiveZoneV1 =
  "A";
...
function cncVmcInjectMachineSelectionStylesV1() {

  if (
    document.getElementById(
      "cnc-vmc-machine-selection-style-v1"
    )
  ) {
    return;
  }
  ...
  style.textContent = `
    ...
    .cnc-zone-tab-v1:last-child {
      border-right:0;
    }


    .cnc-zone-tab-v1.active {
      background:#173d68;
      color:#ffffff;
    }


    .cnc-zone-tab-v1 small {
      display:block;
    ...
    .cnc-machine-button-v1.selected {
    ...
    .cnc-machine-selected-v1.show {
      display:flex;
    }


    .cnc-machine-selected-v1 strong {
      display:block;
...
/* ----------------------------------------------------------
 * FETCH USER TYPE + MACHINES
 * ---------------------------------------------------------- */

async function cncVmcFetchMachineSelectionV1() {
  ...
  if (
    ...
    throw new Error(
      data.error
      || "Unable to load Machine Selection."
    );
  }
...
function cncVmcSelectZoneV1(
  ...
  sessionStorage.setItem(
    CNC_VMC_SELECTED_ZONE_KEY_V1,
    cncVmcActiveZoneV1
  );


  cncVmcRenderMachineSelectionV1();
}
 ...
 *
 * MODULE 1:
 * Only select/verify machine.
 *
 * MODULE 2:
 * This will open dedicated Machine OEE page.
 * ---------------------------------------------------------- */

function cncVmcSelectMachineV1(
  machineId
) {

  const data =
    cncVmcMachineSelectionDataV1;
  ...
  sessionStorage.setItem(
    CNC_VMC_SELECTED_MACHINE_KEY_V1,
    JSON.stringify(
      selectedMachine
    )
  );
  ...
  /*
   * For Module 1 validation only.
   */
  cncVmcRenderMachineSelectionV1();


  showToast(
    `${selectedMachine.machine_no} selected.`,
    "success"
  );
}
...
/* ----------------------------------------------------------
 * RENDER CNC/VMC THIRD VIEW
 * ---------------------------------------------------------- */

function cncVmcRenderMachineSelectionV1() {
  ...
  cncVmcInjectMachineSelectionStylesV1();
  ...
  operatorView.innerHTML = `
    ...
    <div class="cnc-machine-page-v1">

      <div class="cnc-machine-header-v1">
        ...
        <div
          class="cnc-machine-header-badge-v1"
        >
          MACHINE-WISE OEE
        </div>
      ...
      <div class="cnc-zone-card-v1">

        <div class="cnc-zone-tabs-v1">
          ${tabsHtml}
        </div>


        <div class="cnc-machine-content-v1">

          <div class="cnc-zone-title-v1">

            <strong>
              Zone ${esc(activeZone.zone)}
            </strong>

            <span>
              ${
                Number(
                  activeZone.machine_count
                  || 0
                )
              }
              Active Machines
          ...
          ${
            ...
            `
              <div
                class="cnc-machine-empty-v1"
              >
                No active CNC/VMC machines
          ...
          >
            ...
            <div>
              ...
              <small>
                ...
                ${
                  selectedMachine
                    ? esc(
                        `${selectedMachine.machine_name} ? Zone ${selectedMachine.zone}`
                      )
                    : ""
                }
            ...
            <div
              class="cnc-machine-selected-check-v1"
            >
              ?
            </div>
...
/* ----------------------------------------------------------
 * THIRD VIEW DETECTION
 *
 * Existing Normal Operator loader is preserved.
 * We intercept it only when backend says this user
 * has CNC/VMC process access.
 * ---------------------------------------------------------- */

const loadOperatorViewBaseCncMachineV1 =
  loadOperatorView;


loadOperatorView =
  async function loadOperatorViewCncMachineV1() {

    try {

      const data =
        await cncVmcFetchMachineSelectionV1();


      if (
        data.is_cnc_vmc_operator
      ) {
       ...
       * Safety:
       * if detection fails,
       * preserve old operator view.
       */
    }


    return loadOperatorViewBaseCncMachineV1();
  };


/* ----------------------------------------------------------
 * ALSO PROTECT THE NEWER OPERATOR REFRESH FUNCTION
 * ---------------------------------------------------------- */
...
if (
  ...
  const operatorRefreshWorkspaceBaseMachineV1 =
    operatorRefreshWorkspaceV2;


  operatorRefreshWorkspaceV2 =
    async function operatorRefreshWorkspaceMachineV1(
      showFeedback = false
    ) {
      ...
      try {
        ...
        console.error(
          "CNC/VMC workspace refresh:",
          error
        );
      }


      return operatorRefreshWorkspaceBaseMachineV1(
        showFeedback
      );
    };
}
...
/* CNC_VMC_MACHINE_SELECTION_VIEW_V1_END */

/* MACHINE_OEE_PAGE_NAVIGATION_V1_START */


/*
 * Module 2 - Step 1
 *
 * Clicking a CNC/VMC machine now opens
 * its dedicated Machine OEE workspace.
 */


cncVmcSelectMachineV1 =
  function cncVmcSelectMachineOpenOeePageV1(
    machineId
  ) {
    ...
    /*
     * Keep selection only for convenience.
     * Backend still validates machine again.
     */
    sessionStorage.setItem(
      CNC_VMC_SELECTED_MACHINE_KEY_V1,
...
/* MACHINE_OEE_PAGE_NAVIGATION_V1_END */

/* OEE_ZONE_MACHINE_SELECTION_VIEW_V1_START */


let oeeZoneWorkspaceV1 =
  null;
...
async function oeeZoneLoadWorkspaceV1() {
...
function oeeZoneRenderMachineSelectionV1(
  data
) {

  /*
   * Reuse the existing CNC/VMC machine-selection
   * card/grid styling.
   */
  if (
    typeof cncVmcInjectMachineSelectionStylesV1
    === "function"
  ) {
    cncVmcInjectMachineSelectionStylesV1();
  }
  ...
  /*
   * Keep existing cncVmcSelectMachineV1 navigation
   * compatible by supplying expected structure.
   */
  cncVmcMachineSelectionDataV1 = {
    zones:[
      {
    ...
    <div class="cnc-machine-page-v1">
      ...
      <div class="cnc-machine-header-v1">
        ...
        <div>
          ...
          <h2>
            Zone ${esc(zone)} - Select Machine
          </h2>

          <p>
            Select the machine you are
      ...
      <div class="cnc-zone-card-v1">
        ...
        <div class="cnc-machine-content-v1">
          ...
          ">
            ...
            JMS will return here so the next
            operator selects the correct machine.
...
/* ----------------------------------------------------------
 * INTERCEPT OPERATOR VIEW FIRST FOR ZONE ACCOUNTS
 * ---------------------------------------------------------- */

const loadOperatorViewBaseZoneLoginV1 =
  loadOperatorView;


loadOperatorView =
  async function loadOperatorViewZoneLoginV1() {

    try {

      const zoneData =
        await oeeZoneLoadWorkspaceV1();


      if (
        zoneData.is_zone_oee_login
      ) {
        ...
        oeeZoneRenderMachineSelectionV1(
          zoneData
        );
    ...
    return loadOperatorViewBaseZoneLoginV1();
  };
...
if (
  ...
  const operatorRefreshBaseZoneLoginV1 =
    operatorRefreshWorkspaceV2;


  operatorRefreshWorkspaceV2 =
    async function operatorRefreshZoneLoginV1(
      showFeedback = false
    ) {
      ...
      return operatorRefreshBaseZoneLoginV1(
        showFeedback
      );
    };
}
...
/* OEE_ZONE_MACHINE_SELECTION_VIEW_V1_END */



/* ==========================================================
   OEE_ZONE_NATIVE_JMS_VIEW_V2
   ========================================================== */


/* ----------------------------------------------------------
   Sidebar:
   Zone account is doing OEE, not normal Traceability.
   ---------------------------------------------------------- */

function oeeZoneSetSidebarActiveV2() {
...
/* ----------------------------------------------------------
   Convert Page 3 shell from Traceability ? OEE
   ONLY when shared Zone login is active.
   ---------------------------------------------------------- */

function oeeZonePrepareNativeShellV2(
    data
) {
    ...
    if (pageHeader) {
        ...
        subtitle.textContent =
            `Zone ${zone} - Select machine for production entry`;
    }


    /*
     * Hide normal Traceability search.
     *
     * This is the main visual problem in the screenshot.
     */
    const searchCard =
        document.querySelector(
            ".main > .search-card"
    ...
    /*
     * Hide normal Traceability empty state/results.
     */
    const placeholder =
        document.getElementById(
            "placeholder"
...
/* ----------------------------------------------------------
   ...
   Supports current zone-workspace response and the older
   grouped response structure.
   ---------------------------------------------------------- */

function oeeZoneMachinesV2(
    data
) {
...
/* ----------------------------------------------------------
   NATIVE ZONE RENDERER

   Replaces the old custom:
     Zone A - Select Machine card
     +
     Zone A Machines card

   with ONE normal Page-3 card.
   ---------------------------------------------------------- */

window.oeeZoneRenderMachineSelectionV1 =
    function oeeZoneRenderNativeJmsV2(
        data
    ) {

        oeeZonePrepareNativeShellV2(
            data
        );
        ...
        /*
         * Preserve existing global data because
         * cncVmcSelectMachineV1() uses it.
         */
        window.cncVmcMachineSelectionDataV1 =
            data;
        ...
        if (!machines.length) {
            ...
            operatorView.innerHTML = `
                ...
                <div class="search-card">
                    ...
                    <div
                        class="oee-zone-native-note-v2"
                    >
                        No active CNC / VMC machines
                        are available for Zone
                        ${oeeZoneEscapeV2(zone)}.
                    </div>
                ...
                function (machine) {
                    ...
                    return `
                        ...
                        >
                            ...
                            <span
                                class="oee-zone-machine-info-v2"
                            >

                                <span
                                    class="oee-zone-machine-no-v2"
                                >
                                    ${oeeZoneEscapeV2(
                                        machineNo
                                    )}
                                ...
                                <span
                                    class="oee-zone-machine-name-v2"
                                >
                                    ${oeeZoneEscapeV2(
                                        machineName
                                    )}
                            ...
                            <span
                                class="oee-zone-machine-type-v2"
                            >
                                ${oeeZoneEscapeV2(
                                    category
                                )}
        ...
        operatorView.innerHTML = `
            ...
            <div
                class="
                    search-card
                    oee-zone-native-card-v2
                "
            >

                <div
                    class="oee-zone-native-top-v2"
                >
                    ...
                    <div
                        class="oee-zone-native-count-v2"
                    >
                        Zone
                        ${oeeZoneEscapeV2(zone)}
                        
<i
    class="fa fa-industry oee-zone-native-count-icon-v3"
    aria-hidden="true"
></i>

                        ${machines.length}
                        Active Machine${machines.length === 1 ? "" : "s"}
                    </div>
                ...
                <div
                    class="oee-zone-native-note-v2"
                >
                    Select the machine where the operator
                    is entering production.
                </div>
...
/* ==========================================================
   OEE_ZONE_NATIVE_JMS_VIEW_V2_END
   ========================================================== */



/* ==========================================================
   OEE_ZONE_MACHINE_CLICK_FIX_V4
   ========================================================== */


window.oeeZoneNativeMachinesV4 =
    [];
...
function oeeZoneEscapeV4(value) {
...
/* ----------------------------------------------------------
   NORMALIZE ZONE MACHINE RESPONSE
   ---------------------------------------------------------- */

function oeeZoneMachineListV4(data) {
    ...
    if (
        ...
        for (const zoneRow of data.zones) {
...
/* ----------------------------------------------------------
   OPEN SELECTED MACHINE
   ---------------------------------------------------------- */

function oeeZoneOpenMachineV4(
    machineId
) {
    ...
    if (!machine) {
        ...
        alert(
            "Machine data is not available. Refresh the page and try again."
        );
    ...
    /*
     * No old machine-data lookup here.
     *
     * Dedicated backend page validates that this
     * machine belongs to the logged-in Zone.
     */
    window.location.href =
...
/* ----------------------------------------------------------
   REPLACE ZONE MACHINE RENDERER
   ---------------------------------------------------------- */

window.oeeZoneRenderMachineSelectionV1 =
    function oeeZoneRenderNativeJmsV4(
        data
    ) {

        if (
            typeof oeeZonePrepareNativeShellV2
            === "function"
        ) {
        ...
        const machines =
            oeeZoneMachineListV4(
                data
            );


        /*
         * This becomes the machine authority for
         * the current Zone selection screen.
         */
        window.oeeZoneNativeMachinesV4 =
            machines.slice();
            ...
            operatorView.innerHTML = `
                ...
                <div class="search-card">
                    ...
                    >
                        ...
                        No active CNC or VMC machines
                        are available for Zone
                        ${oeeZoneEscapeV4(zone)}.
                ...
                function(machine) {
                    ...
                    return `
                        ...
                        >
                            ...
                            <span
                                class="
                                    oee-zone-machine-info-v2
                                "
                            >

                                <span
                                    class="
                                        oee-zone-machine-no-v2
                                    "
                                >
        ...
        operatorView.innerHTML = `
            ...
            >
                ...
                >
                    ...
                    Select the machine where the
                    operator is entering production.
...
/* ==========================================================
   OEE_ZONE_MACHINE_CLICK_FIX_V4_END
   ========================================================== */
